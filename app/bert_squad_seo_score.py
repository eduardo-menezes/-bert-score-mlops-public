...
# -*- coding: utf-8 -*-
# """Bert_Squad_SEO_Score_Colab.ipynb

# Automatically generated by Colaboratory..

# Original file is located at
# https://colab.research.google.com/drive/1Kc0AmfVDqNL7Wif9wczq71X8T575I8Kn
...
def main():
    ...
    class SquadExample(object):
        ...
        # For examples without an answer, the start and end position_serp are -1.
        # """
        ...
        def __init__(self,
                    ...
                    start_position_serp=None,
                    end_position_serp=None):
            self.qas_id = qas_id
            self.question_text = question_text
            self.doc_tokens = doc_tokens
            self.orig_answer_text = orig_answer_text
            self.start_position_serp = start_position_serp
            self.end_position_serp = end_position_serp
        ...
        def __repr__(self):
            ...
            if self.start_position_serp:
                s += ", start_position_serp: %d" % (self.start_position_serp)
            if self.end_position_serp:
                s += ", end_position_serp: %d" % (self.end_position_serp)
            return s
    ...
    def input_to_squad_example(passage, question):
        ...
        example = SquadExample(
            ...
            start_position_serp=start_position_serp,
            end_position_serp=end_position_serp)
                    
        return example

    def _check_is_max_context(doc_spans, cur_span_index, position_serp):
        ...
        for (span_index, doc_span) in enumerate(doc_spans):
            ...
            if position_serp < doc_span.start:
                continue
            if position_serp > end:
                continue
            num_left_context = position_serp - doc_span.start
            num_right_context = end - position_serp
    ...
    def get_final_text(pred_text, orig_text, do_lower_case, verbose_logging=False):
        ...
        start_position_serp = tok_text.find(pred_text)
        if start_position_serp == -1:
            return orig_text
        end_position_serp = start_position_serp + len(pred_text) - 1
        ...
        orig_start_position_serp = None
        if start_position_serp in tok_s_to_ns_map:
            ns_start_position_serp = tok_s_to_ns_map[start_position_serp]
            if ns_start_position_serp in orig_ns_to_s_map:
                orig_start_position_serp = orig_ns_to_s_map[ns_start_position_serp]

        if orig_start_position_serp is None:
            return orig_text

        orig_end_position_serp = None
        if end_position_serp in tok_s_to_ns_map:
            ns_end_position_serp = tok_s_to_ns_map[end_position_serp]
            if ns_end_position_serp in orig_ns_to_s_map:
                orig_end_position_serp = orig_ns_to_s_map[ns_end_position_serp]

        if orig_end_position_serp is None:
            return orig_text

        output_text = orig_text[orig_start_position_serp:(orig_end_position_serp + 1)]
        return output_text
    ...
    #text area
    myKeyword = st.text_area("coloque a mensagem",value= 'O que é marketing digital?',height=100)
    st.write(myKeyword)
    ...
    """
    Um segundo, estamos buscando nos 10 primeiros sites do google.
    """

    dfScrap = pd.DataFrame(columns=['keyword', 'page', 'position_serp', 'BERT_score', 'source', 'search_date'])
    myNum=10
    myStart=0
    ...
    while myStart <  myMaxStart:
        ...
        try  : 
            ...
            df = pd.DataFrame(columns=['keyword', 'page', 'position_serp', 'BERT_score', 'source', 'search_date'])
            for url in urls :
                print("URL:"+url)
            ...
            df['position_serp'] = df.index.values + 1 + myStart #position_serp = index +1 + myStart
    ...
    """Fizemos a busca e agora vamos calcular qual site, contextualmente, melhor responde a sua pergunta"""
    ...
    dfPagesAnswers = pd.DataFrame(columns=['keyword', 'page', 'position_serp', 'BERT_score', 'source', 'search_date','answers','starts', 'ends', 'local_probs', 'total_probs'])
    ...
    for i in range(0,len(dfPagesUnique)) :
        ...
        if len(soup) != 0 :
            ...
            dfPageAnswer = pd.DataFrame(columns=['keyword', 'page', 'position_serp', 'BERT_score', 'source', 'search_date','answers','starts', 'ends', 'local_probs', 'total_probs'])
            for k in range (0, len(dfAnswer)) :
                ...
                dfPageAnswer.loc[k, 'position_serp'] = dfPagesUnique.loc[i,  'position_serp']
    ...
    #st.dataframe(dfPagesAnswers.sort_values(by='BERT_score', ascending=False).head(50))
    ...
    dfPagesSummary = dfPagesUnique[['keyword', 'page', 'position_serp', 'BERT_score', 'search_date']]
    ...
    st.dataframe(dfPagesSummary.sort_values(by='BERT_score', ascending=False).head(50))
    
    return print('terminou')
# percent_complete = i/len(dfPagesUnique)
# my_bar.progress(value = 0 <= percent_complete + 1 <= 1)    
